...
inline QString to_scm_str(QStringList assertions,int templt,QString linktype,const QList<float>& meanlist,const QStringList& wlist)
{
    ...
    QString cn1 = "(ConceptNode \""+cn_argument1+"\" ("+QString::number(mean)+" "+QString::number(confidence)+"))";

    QString cn_argument2 = assertions[2].mid(6);
    i =wlist.indexOf(cn_argument2);
    mean = (i !=-1) ? meanlist[i] : 0.0;
    confidence = 0.95;
    QString cn2 = "(ConceptNode \""+cn_argument2+"\" ("+QString::number(mean)+" "+QString::number(confidence)+"))";
    ...
    if(templt == 1){
        ...
        QString pn = "(PredicateNode \""+cn_relation+"\" ("+QString::number(confidence_value)+" "+QString::number(0.9)+"))";
        QString listlink = "(ListLink\n  "+cn1+"\n  "+cn2+")";
        QString evallink = "(EvaluationLink ("+QString::number(confidence_value)+"  "+QString::number(0.9)+")\n  "+pn+"\n  "+listlink+")\n";
        return evallink;
    }
    return QString();
}
